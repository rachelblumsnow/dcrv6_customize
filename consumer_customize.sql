...
insert into dcr_DEMOCUR_consumer.local.user_settings (setting_name, setting_value)
 VALUES ('app_data','dcr_DEMOCUR_data'),
        ('app_data_two','dcr_DEMOCUR_data_two'),
        ('consumer_db','dcr_DEMOCUR_consumer'),
        ('consumer_schema','mydata'),
        ('consumer_table','customers'),
        ('consumer_join_field','email'),
        ('app_instance','dcr_DEMOCUR_app'),
        ('app_instance_two','dcr_DEMOCUR_app_two'),
        ...
        ('consumer_requests_table','dcr_DEMOCUR_consumer.shared.requests'),
...
//call dcr_DEMOCUR_app.cleanroom.request('audience_overlap',
//object_construct(
...
select REQUEST_ID, APPROVED, request:PROPOSED_QUERY::varchar from dcr_DEMOCUR_app.cleanroom.provider_log;
...
select
    ...
    , to_number(round(dcr_DEMOCUR_app.cleanroom.addNoise(count(distinct p.user_id),0.01,dcr_DEMOCUR_app.cleanroom.ns_306ef26f_3ce4_449f_bbea_8fb453473436()))) as dp_overlap 
from
    dcr_DEMOCUR_data.cleanroom.provider_publisher p,
    dcr_DEMOCUR_consumer.mydata.brand_customer at(timestamp => '2022-04-06 12:49:03.331 -0700'::timestamp_tz) c
where
    c.user_id = p.user_id
    and exists (select table_name from dcr_DEMOCUR_consumer.information_schema.tables where table_schema = upper('mydata') and table_name = upper('customers') and table_type = 'BASE TABLE')
    and ((select any_value(pid) from dcr_DEMOCUR_data.cleanroom.pid)=(select any_value(pid) from dcr_DEMOCUR_app.cleanroom.pid))
group by
    identifier('c.marital')
    , identifier('c.age_range')
    , identifier('c.lifestyle_like')
